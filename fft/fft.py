...
(inputSignal, samplingRate, bits) = audio.wavread(filename+'_ent.wav')
inputSignal = numpy.array([x/2.0 + random.random()*0.1 for x in inputSignal])

noisyFile = audio.Sndfile(filename+'_noisy.wav', 'w', audio.Format('wav'), 1, samplingRate)
noisyFile.write_frames(inputSignal)
noisyFile.close()

outputSignal = processWithOurFFT(inputSignal)

outputFile = audio.Sndfile(filename+'_transformed.wav', 'w', audio.Format('wav'), 1, samplingRate)
